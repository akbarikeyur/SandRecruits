...
//MARK: - Fetch Image
func setButtonBackgroundImage(_ picPath:String, btn:[UIButton]) {
    if picPath == ""
    {
        return
    }
    for i in 0..<btn.count{
        btn[i].sd_setBackgroundImage(with: URL(string : picPath), for: .normal, completed: { (image, error, caheType, url) in
            if error == nil
            {
...
func setImageViewBackgroundImage(_ picPath:String, imgView:[UIImageView]) {
    if picPath == ""
    {
        return
    }
    for i in 0..<imgView.count{
        ...
        imgView[i].sd_setImage(with: URL(string : picPath)) { (image, error, cacheType, url) in
            if error == nil
            {
