...
//@IBDesignable
 class TextField: UITextField {
    ...
    @IBInspectable var fontTypeAdapter : String = FontType.MontserrstRegular.value {
        didSet {
            self.fontType = FontType(rawValue: self.fontTypeAdapter)!
        }
    }
    var fontType : FontType = FontType.MontserrstRegular {
        didSet {
    ...
    @IBInspectable var placeholderColorString : String = "" {
        didSet {
            self.attributedPlaceholder = NSAttributedString(string: placeholderColorString, attributes: [NSAttributedStringKey.foregroundColor: UIColor.lightGray])
        }
    }
